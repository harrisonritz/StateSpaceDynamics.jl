...
better-converged posterior at proportional cost per iteration. The discrete update
averages over `num_samples` draws from ``q(x)``. The two knobs do different things:
raising `num_samples` reduces the Monte-Carlo noise in the discrete update, while
raising `smoothing_iters` gives the posterior more time to converge but does not
average that noise away.

## Post-fit inference

[`smooth`](@ref) infers the continuous states, discrete-state responsibilities, and
ELBO with the fitted parameters held fixed. It alternates forward-backward over the
switching chain with the Laplace/Kalman smoother over the continuous states until `γ`
converges (`tol`) or `smoothing_iters` alternations are spent — up to 100 by default,
since a standalone inference call is not inside an EM loop and is expected to converge.

Unlike `fit!`, its discrete update takes no draws from ``q(x)``: it expands
``E_{q(x)}[\log p]`` about the smoothed mean and adds the second-order term
``\tfrac12 \mathrm{tr}(H\Sigma)``, so repeated calls are deterministic. That expansion
is exact for Gaussian emissions, making the returned ELBO monotone in `smoothing_iters`
up to roundoff; for Poisson emissions it is second-order and small excursions remain.

```@docs; canonical = false
smooth(slds::SLDS{T,S,O}, y::StateSpaceDynamics.CompositeObservations{T}) where {T<:Real,S<:AbstractStateModel,O<:AbstractObservationModel}
```
...
```@docs; canonical = false
smooth(slds::SLDS{T,S,O}, y::Union{AbstractMatrix{T},AbstractArray{T,3},AbstractVector{<:AbstractMatrix{T}},NamedTuple}) where {T<:Real,S<:AbstractStateModel,O<:AbstractObservationModel}
```
