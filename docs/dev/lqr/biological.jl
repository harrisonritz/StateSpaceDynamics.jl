...
function section_identify()
    ...
    println()
    println("The structural lesson: every slack model loads on range(B) — here the")
    println("velocity subspace — so an emission that does not span the control directions")
    println("cannot see suboptimality at any sample size.")
    println()
    println("DO NOT read this table as a comparison BETWEEN the two slack models. It")
    println("compares them at the same NOMINAL scalar, which is nowhere near the same")
    println("observable effect: at sigma_nu = 1 the max-ent term carries 86% of the")
    println("innovation variance and the Riccati term carries 0.02%. Run --only=threshold,")
    println("which sweeps the magnitude and scores on the share phi the data can see; at")
    println("matched phi the max-ent tying has no advantage in this design.")
end

function section_threshold()
    section("threshold — how large must the slack be before it is detectable?")
    println("φ = share of the innovation variance contributed by the slack term — the")
    println("quantity the data can actually see, and the right axis for a power comparison.")
    println()
