...
function _slds_terminal_gradient!(
    ...
    # grad[:, T] -= w · Λfᵀ Σf⁻¹ r_f
    @views mul!(grad[:, tsteps], sm.cache.LtSinv[kf], rf, -w_k[tsteps], one(T))
    return nothing
end
...
function smooth!(
    ...
    if x_sample !== nothing
        ...
        trial its own generator. With `num_samples > 1` the buffer holds the whole
        trial's draws end to end, `n_active` standard normals per sample.
        =#
        for s in axes(x_sample, 3)
            if noise === nothing
                randn!(rng, z)
            else
                copyto!(z, view(noise, ((s - 1) * n_active + 1):(s * n_active)))
            end
...
deterministic path); otherwise the next draws from `q(x)` land in
`x_samples[trial]`, one per slice of its third (sample) axis.
...
function _slds_smooth_all!(
    ...
    y::Union{AbstractVector{<:AbstractMatrix{T}},NamedTuple},
    x_samples::Union{Nothing,AbstractVector{<:AbstractArray{T,3}}},
    pool::SLDSWorkspacePool{T},
    plan::SLDSTrialPlan,
    w_of;
...
- `x_samples !== nothing` — plug in a joint draw from `q(x)`, and draw the next one in
  ...
  read then overwritten within each alternation. Its third axis is the sample axis:
  with more than one slice, step 1 scores each draw and averages the per-regime
  log-densities, which is the `num_samples` estimate of `E_q(x)[log p_k]`.
...
function _vem_alternate!(
    ...
    #=
    Deterministic path scores the smoothed mean; the sampled path scores draw `s`
    from q(x). With `num_samples > 1` the per-regime log-densities are averaged
    over the draws — the Monte-Carlo estimate of E_q(x)[log p] the discrete update
    wants. It is the log-densities that are averaged, not the trajectories:
    averaging the paths first would score a shrunk trajectory that no draw
    visited, which is the plug-in the deterministic path already offers.
    =#
    ...
    for iter in 1:smoothing_iters
        ...
        if !all(isfinite, dl.logL)
            counts = [count(x -> !isfinite(x), view(dl.logL, k, :)) for k in 1:K]
            nans = [count(isnan, view(dl.logL, k, :)) for k in 1:K]
...
`num_samples` is how many draws from `q(x)` the discrete update averages over.
The two knobs address different things: `num_samples` shrinks the Monte-Carlo
noise in the discrete update (as `1/√num_samples`), while `smoothing_iters`
gives the posterior more alternations to converge and does not average that
noise away. Each extra sample costs one more scoring pass over the trials per
alternation — it does not re-run the smoother, which writes every draw on its
way out. The draws are averaged as log-densities, not as trajectories.

Reproducibility follows `rng_mode`, sample axis included: `:trial` seeds each
trial's generator from `rng`, and `:global` pre-draws the whole stream serially
in trial order, so neither depends on how the parallel pass is scheduled.
...
function fit!(
    ...
    #=
    One buffer per trial, `latent_dim × T × num_samples`. The discrete update
    averages the per-regime log-densities over the sample axis; the continuous
    smoother writes every draw on its way out, so the extra draws cost the
    sampling step and the extra scoring passes, not another smooth.
    =#
