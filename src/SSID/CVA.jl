# =============================================================================
# CVA.jl — Canonical Variate Analysis (CVA) subspace-identification initializer
#
...
# method for the Gaussian LTI LDS, invoked via `fit!(lds, y, CVA(); …)`.
#
# CVA produces a deterministic, data-driven estimate of (A, C, Q, R, b, d, x0,
# P0, B, D) that seeds EM. It does NOT run EM. Scope is intentionally limited to
# `GaussianStateModel` + `GaussianObservationModel`; other model types hit a
# fallback method that throws.
#
# Dependency-free port: the reference uses ControlSystems (`lsim`/`ss`) and
# MatrixEquations (`ared`). Here those are replaced with pure-Julia kernels —
# `_ltisim` (discrete-time LTI simulation) and `_dlyap` (discrete Lyapunov solve
# for the stationary state covariance used as `P0`).
# =============================================================================

"""
    CVA(; r=0, s1=0, s2=0, W=:CVA, zeroD=true, stable=true, scaleU=false,
...
```julia
diag = fit!(lds, y, CVA())                 # autonomous LDS
diag = fit!(lds, y, CVA(); control_seq=u)  # with dynamics inputs (estimates B)
```
...
function _ltisim(
    A::AbstractMatrix{T}, E::AbstractMatrix{T}, C::AbstractMatrix{T},
    u::AbstractMatrix{T}, x0::AbstractVector{T},
) where {T}
    n = size(A, 1)
    p = size(C, 1)
    N = size(u, 2)
    Y = Matrix{T}(undef, p, N)
    x = Vector{T}(undef, n); copyto!(x, x0)
    xn = Vector{T}(undef, n)
...
# -----------------------------------------------------------------------------
# CVA subspace machinery (block-Hankel construction + weighting + recovery)
# -----------------------------------------------------------------------------

# Forward block-Hankel matrix (r*d × N) from time-major data (t × d).
function _cva_hankel(data::AbstractMatrix{T}, t0::Int, r::Int, N::Int) where {T}
    d = size(data, 2)
    H = zeros(T, r * d, N)
...
function _cva_phi(
    y::AbstractMatrix{T}, u::AbstractMatrix{T}, t0::Int, N::Int,
    s1::Int, s2::Int, p::Int, m::Int,
) where {T}
    s = s1 * p + s2 * m
    Φ = zeros(T, s, N)
...
# variance explained `fve`, and the L-blocks (L1, L2) needed by `_find_PK_cva`.
function _cva_subspace(
    Y::AbstractMatrix{T}, U::AbstractMatrix{T}, Φ::AbstractMatrix{T},
    r::Int, s1::Int, s2::Int, p::Int, m::Int, W::Symbol, n::Int,
) where {T}
    ...
    elseif W === :CVA
        ...
        throw(ArgumentError("CVA: unknown weighting W=:$(W); expected :CVA, :MOESP, :N4SID, or :IVM"))
    end
...
function _cva_AC(Or, p::Int, r::Int, n::Int, Aestimator, stable::Bool)
    C = Or[1:p, 1:n]
...
function _find_BD_cva(
    A::AbstractMatrix{T}, C::AbstractMatrix{T}, U::AbstractMatrix{T},
    V::AbstractMatrix{T}, Y::AbstractMatrix{T}, zeroD::Bool, estimator, ::Type{T},
) where {T}
    n = size(A, 1)
    p = size(C, 1)
    N = size(Y, 2)
...
function _cva_fit!(
    ...
    n == lds.latent_dim || throw(
        ArgumentError("CVA: order ($n) must equal lds.latent_dim ($(lds.latent_dim))"),
    )
    ...
    r >= n + 1 || throw(ArgumentError(
        "CVA: prediction horizon r=$r must be ≥ model order n+1=$(n + 1)"))
    alg.verbose && @info "CVA: r=$r, s1=$s1, s2=$s2, W=:$(alg.W), n=$n, cross_trial=$(alg.cross_trial)"
    ...
    if alg.cross_trial
        ...
        N > 0 || throw(ArgumentError(
            "CVA: not enough data (Ttot=$Ttot) for r=$r; need Ttot > $(r - 1 + t0)"))
        Yh = _cva_hankel(ytm, t0, r, N)
        Uh = _cva_hankel(utm, t0, r, N)
        Φh = _cva_phi(ytm, utm, t0, N, s1, s2, p, m)
    else
        Yblocks = Matrix{T}[]
        Ublocks = Matrix{T}[]
        Φblocks = Matrix{T}[]
        ...
        for (i, yi) in enumerate(y)
            Ti = size(yi, 2)
            Ni = Ti - r + 1 - t0
            Ni > 0 || throw(ArgumentError(
                "CVA: trial $i too short (T=$Ti) for r=$r with cross_trial=false; need T > $(r - 1 + t0)"))
            ytm = permutedims(yi .- ȳ)
            utm = m > 0 ? permutedims(u_seq[i]) : zeros(T, Ti, 0)
            if alg.scaleU && m > 0
                utm = utm ./ _colstd(utm)'
            end
            push!(Yblocks, _cva_hankel(ytm, t0, r, Ni))
            push!(Ublocks, _cva_hankel(utm, t0, r, Ni))
            push!(Φblocks, _cva_phi(ytm, utm, t0, Ni, s1, s2, p, m))
            col0 += Ni
        end
        col0 > 0 || throw(ArgumentError("CVA: no usable Hankel columns (data too short)"))
    ...
    size(Yh, 2) >= block_rows || throw(ArgumentError(
        "CVA: too few Hankel columns ($(size(Yh, 2))) for r=$r, s1=$s1, s2=$s2; " *
        "need ≥ $block_rows. Reduce r/s1/s2 or provide more data."))

    # Subspace identification → Or, A, C, Q, R.
    Or, sv, fve, L1, L2 = _cva_subspace(Yh, Uh, Φh, r, s1, s2, p, m, alg.W, n)
...
"""
    fit!(lds, y, alg::CVA; control_seq=nothing, obs_control_seq=nothing)
...
`(obs_dim, T, ntrials)` array. Returns `(; S, fve)`. See [`CVA`](@ref).
"""
function fit!(
    lds::LinearDynamicalSystem{T,S,O},
    y::AbstractVector{<:AbstractMatrix{T}},
    ...
    return _cva_fit!(lds, y, u_seq, v_seq, alg)
end

function fit!(
    lds::LinearDynamicalSystem{T,S,O}, y::AbstractArray{T}, alg::CVA; kwargs...
) where {T<:Real,S<:GaussianStateModel{T},O<:GaussianObservationModel{T}}
    if ndims(y) == 3
    ...
    elseif ndims(y) == 2
        ...
        throw(ArgumentError("CVA: input array y must be 2D or 3D, got ndims=$(ndims(y))."))
    end
end

# Fallback for unsupported model types (Poisson, stitched, …) — narrower Gaussian
# methods above take precedence when applicable.
function fit!(lds::LinearDynamicalSystem, y, alg::CVA; kwargs...)
    throw(
        ArgumentError(
            "CVA initialization currently supports only Gaussian LTI LDS " *
