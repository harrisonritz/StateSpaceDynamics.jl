...
- `fit!(slds, y; smoothing_iters=n)`: run `n` discrete↔continuous alternations
  ...
  cost per iteration
- `fit!(slds, y; num_samples=n)`: average the discrete update over `n` draws
  from `q(x)` instead of one. It addresses a different quantity than
  `smoothing_iters`: `num_samples` shrinks the Monte-Carlo noise in the discrete
  update, while `smoothing_iters` gives the posterior more alternations to
  converge and does not average that noise away. Each extra draw costs one more
  scoring pass per alternation, not another smooth — the smoother writes every
  draw on its way out — and the draws are averaged as log-densities, not as
  trajectories. Reproducible under both `rng_mode`s, sample axis included (#161)
